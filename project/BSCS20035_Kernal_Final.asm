...
insertThread:
		...
		xor cx,cx
		mov cl,byte[cs:pcb]							;save next of 0 thread

		mov bx,[bp+4]
		mov dx,bx									;save current thread
		shl bx,5

		mov al,cl			
		mov byte[cs:pcb+bx],al						;set next of current thread
		mov byte[cs:pcb+bx+1],0						;set prev of current thread

		mov byte[cs:pcb],dl							; set current ,next of 0 pcb

		mov bl,cl									; go to next of 0 pcb
		mov bh,0
		shl bx,5
		mov byte[cs:pcb+bx+1],dl					;set prev of next of pcb
...
removeThread:
		...
		mov cl,[cs:pcb+bx]					;next of current pcb												prev pcbnum<->current num<->next pcb num
		mov dl,[cs:pcb+bx+1]				;prev of current pcb


		mov si,dx							; go to prev of current pcb
		shl si,5
		mov byte[cs:pcb+si],cl				; set next of prev of currentpcb , to the next of current pcb

		mov si,cx							; go to next of current pcb
		shl si,5
		mov byte[cs:pcb+si+1],dl			;set prev of next of pcb,to the prev of current pcb

		mov word[cs:pcb+bx+2],0				;pcb is now free
